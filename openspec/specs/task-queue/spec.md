...
Define manutenção e consistência da fila persistente de tarefas.
...
### Requirement: Estados localizados na interface
O sistema SHALL manter os estados técnicos estáveis no domínio e SHALL apresentar os estados da fila em português na interface.

#### Scenario: Fila visível no navegador
- **WHEN** uma tarefa está pendente, em votação ou concluída
- **THEN** a interface exibe respectivamente `Pendente`, `Em votação` ou `Concluída`, sem expor o enum em inglês
