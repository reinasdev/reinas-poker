...
describe("versioned initial migration", () => {
  const migration = readFileSync(resolve("drizzle/0000_initial_schema.sql"), "utf8");
  ...
  it("creates every persisted aggregate", () => {
    for (const table of ["users", "magic_codes", "sessions", "rooms", "room_participants", "tasks", "voting_rounds", "votes"]) {
      expect(migration).toContain(`CREATE TABLE \"${table}\"`);
    }
  });

  it("contains enums, indexes, foreign keys and uniqueness constraints", () => {
    for (const enumName of ["room_status", "round_status", "task_status", "voting_style"]) {
  ...
  it("keeps ordered Drizzle metadata for every SQL migration", () => {
    expect(journal.entries.map(({ idx, tag }) => ({ idx, tag }))).toEqual([{ idx: 0, tag: "0000_initial_schema" }]);
  });
