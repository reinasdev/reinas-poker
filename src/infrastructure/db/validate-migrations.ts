...
/** users, rooms, room_participants, tasks, voting_rounds, votes. */
const EXPECTED_TABLES = 6;

const databaseNames = [
  "reinas_poker_migration_empty",
  "reinas_poker_migration_upgrade",
] as const;
...
async function migrateDatabase(url: string, folder: string) {
  ...
  await pool.end();
  return {
    tables: tables.rows[0].count as number,
    migrations: history.rows[0].count as number,
  };
}
...
async function main() {
  ...
  const empty = await migrateDatabase(
    databaseUrl(databaseNames[0]),
    migrationRoot,
  );
  if (empty.tables !== EXPECTED_TABLES || empty.migrations !== sqlFiles.length)
    throw new Error("Empty database migration validation failed");
  ...
  if (sqlFiles.length > 1) {
    ...
    await writeFile(
      resolve(previousRoot, "meta/_journal.json"),
      JSON.stringify(journal),
    );
    for (const file of sqlFiles.slice(0, -1))
      await cp(resolve(migrationRoot, file), resolve(previousRoot, file));
    await migrateDatabase(upgradeUrl, previousRoot);
    await rm(previousRoot, { recursive: true, force: true });
  }
  const upgraded = await migrateDatabase(upgradeUrl, migrationRoot);
  if (upgraded.tables !== EXPECTED_TABLES || upgraded.migrations !== sqlFiles.length)
    throw new Error("Previous-version upgrade validation failed");
  console.info(
    `Validated ${sqlFiles.length} migration(s) on empty and previous-version databases`,
  );
}
